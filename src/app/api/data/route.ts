  ...
  try {
    ...
    for (const bridgeData of bridges) {
      try {
        ...
        for (const span of bridge.spans) {
          ...
          if (spanData && spanData.walkingBoards && spanData.walkingBoards.length > 0) {
            ...
            if (defaultBoards.length > 0) {
              await db.walkingBoard.createMany({ data: defaultBoards })
            }
          }
        }
        ...
        results.failed++
        results.errors.push(`桥梁 "${bridgeData.name || bridgeData.bridgeCode}" 导入失败: ${err instanceof Error ? err.message : '未知错误'}`)
      }
    }
